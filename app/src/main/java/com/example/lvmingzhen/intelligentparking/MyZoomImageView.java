...
/**
 * Created by lvmingzhen on 2018/7/10.
 */
...
public class MyZoomImageView extends ImageView {
     ...
     */
    private void initUI() {
        //this.setScaleType(ScaleType.FIT_CENTER);//把图片按比例扩大/缩小到View的宽度，居中显示，还有几个，分别是
        ...
        this.setOnTouchListener(new TouchListener());//注册监听器
        //ImageView的宽高度大于等于图片的宽度高度
    ...
    //ontouchlistener在手指触摸和离开的时候仅会有反应，执行两次，而onclicklistener只有在手指离开的时候触发一次事件
    private final class TouchListener implements OnTouchListener {
        ...
        /** 用于记录开始时候的坐标位置 */
        private PointF startPoint = new PointF();//数值类型为float类型
        /** 两个手指的开始距离 */
        ...
        private float checkDyBound(float[] values, float dy) {
            ...
            //若图片高度乘以缩放倍数小于控件高度，则不需要操作
            if (intrinsicHeight * values[Matrix.MSCALE_Y] < height)
                return 0;
            //如果将要移动的距离大于其所规定的最大移动距离，则将实际移动距离按照规定的最大移动距离来
